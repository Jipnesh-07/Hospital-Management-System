...
func fetchDoctorAppointments(token: String) {
    ...
    let task = URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        do {
            ...
            print("Success: \(appointmentResponse.success)")
            for appointment in appointmentResponse.data {
                print("Appointment ID: \(appointment._id), Patient: \(appointment.patient.firstName) \(appointment.patient.lastName), Doctor ID: \(appointment.doctor), Date: \(appointment.date), Time Slot: \(appointment.timeSlot.rawValue)")
